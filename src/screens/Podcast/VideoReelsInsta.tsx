...
const ACCESS_TOKEN = "EAAYJ6U4AIAQBO4UCcbd5jZBPioEGZBdZBPrAKEfkiggHmMA5UZBAAX5QA3mdo7oGmy05TUxz9vnFWzqf4GPbNPNWKcvWrgOU48LyGKJ2aEGCUPmIGvCSkP4NvWSxMpWKp2d8VyOX2gZAet2K1dZA9XOb5uWhdU92zUab6LJDXA96ssqM7zdSXFmgZDZD"; // Replace with your token
...
const [videos, setVideos] = useState([]);
  ...
  const fetchVideos = async (url = `${FACEBOOK_API_URL}/${ACCOUNT_ID}/tags?fields=id,media_type,media_url,permalink,timestamp&access_token=${ACCESS_TOKEN}&limit=5`) => {
    try {
      ...
      const filteredVideos = data.data.filter((item) => item.media_type === "VIDEO");
    ...
    <View style={{ height: VIDEO_HEIGHT, width, justifyContent: "center", alignItems: "center" }}>
     {currentIndex==index &&
      <Video
        ...
        onEnd={handleVideoEnd} // Play next video when the current one ends
      />}
    </View>
  ), [currentIndex]);
