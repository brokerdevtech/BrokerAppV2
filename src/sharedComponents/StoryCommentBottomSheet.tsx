...
import { AddComment, AddstoryComment, GetCommentList, GetStoryCommentList, ReplyComment, SetPostCommentLikeUnLike, SetStoryCommentLikeUnLike } from '../../BrokerAppCore/services/new/postServices';
...
const StoryCommentBottomSheet = forwardRef(({ postItem, User, listTypeData,StoryStateParam,userPermissions,onClose }, ref) => {
  ...
  const navigation = useNavigation();
    ...
    } = useApiPagingWithtotalRequest(GetStoryCommentList,setInfiniteLoading,5);
      ...
      <View key={index} style={styles.mainContainer}>
        <View styles={styles.profileWrap}>
          ...
          <ZAvatarInitials
            ...
            name={item.viewerName}></ZAvatarInitials>
        </View>
  ...
  const handleCommentLike = async item => {
    ...
    let endpoint=""
     ...
     if (item?.userLiked && item?.userLiked == 1) {
        ...
        result = await SetStoryCommentLikeUnLike('UnLike',
                  User.userId,
                  item.commentId,
                 );
                 if (result?.success == true) {
                    ...
                    result = await SetStoryCommentLikeUnLike('Like',
                        User.userId,
                        item.commentId,
                       );   
                       if (result?.success == true) {
      ...
      const renderFooter = () => (
        <View style={styles.footerContainer}>
          <HStack style={{ justifyContent: 'center' }}>
            <TextInput style={{ flex: 1 }} placeholder="Add a comment..."
           value={newComment}
        ...
        if (!isInfiniteLoading) {
          ...
          if (replyCommentId === 0) {
           ...
           await  setStoryState({
              ...
              reactionCount: postComment.data.data.storyDetails[0].reactionCount,
