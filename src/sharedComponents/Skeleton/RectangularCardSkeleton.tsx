...
export default function RectangularCardSkeleton({type='broker'}) {
  return (
    <HStack space={'xl'} style={{ marginTop:10,paddingHorizontal:10}}>
      {type=='broker' ?  Array.from({ length: 4 }).map((_, index) => (
               <Box className="w-[400px] gap-4 p-3 rounded-md bg-background-100" style={{ height:140,
                paddingHorizontal:10, 
                paddingVertical:10,
                width:120,
                
               }}>
        <VStack key={index} className="gap-2 align-middle" style={{alignItems:'center'}}>
          <Skeleton 
            variant="circular" 
        ...
        <Box className="w-[400px] gap-4  rounded-lg bg-background-100" style={{ height:170,
 ...
 <VStack key={index} className="gap-2 align-middle" style={{alignItems:'center'}}>
  ...
  <SkeletonText _lines={3} gap={2} className="h-4 w-30" />
    ...
    <Box className="w-[400px] gap-4 p-3 rounded-lg bg-background-100" style={{ height:180,
